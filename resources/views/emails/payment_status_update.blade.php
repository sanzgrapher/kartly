...
@component('emails.layout', [
    ...
    <div class="info-box">
        ...
        <p>Status: <span class="highlight">{{ ucfirst($order->payment->payment_status?->value ?? 'Unknown') }}</span></p>
        <p>Method: {{ ucfirst($order->payment->payment_method?->value ?? 'N/A') }}</p>
        <p>Amount: <span class="highlight">Rs {{ $order->total ?? 0 }}</span></p>
    </div>

    @if (strtolower($order->payment->payment_status?->value ?? '') === 'completed')
        <div
    ...
    @elseif(strtolower($order->payment->payment_status?->value ?? '') === 'pending')
        <div
            style="background-color: #fff7ed; padding: 15px; border-radius: 8px; margin: 20px 0; text-align: center; border-left: 4px solid #f97316;">
            <p style="font-size: 13px; color: #92400e; margin: 0;">
    ...
    @elseif(strtolower($order->payment->payment_status?->value ?? '') === 'failed')
        <div
