...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        ...
        <div>
            ...
            <div class="mb-6">
                <p class="text-4xl font-bold text-orange-600">Rs {{ number_format($product->price, 2) }}</p>
            </div>
            ...
            @auth
                <div class="flex items-center gap-4 mb-6">
                    <input type="number" id="quantity" name="quantity" min="1" max="{{ $product->quantity }}"
                        value="1"
                        class="w-20 px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-orange-500"
                        @if ($product->stock_status == 'Out of Stock') disabled @endif>
                    <button onclick="addToCart({{ $product->id }})"
                        class="flex-1 px-6 py-3 bg-orange-500 text-white font-semibold rounded hover:bg-orange-600 transition disabled:bg-gray-400 disabled:cursor-not-allowed"
                        @if ($product->stock_status == 'Out of Stock') disabled @endif>
                        Add to Cart
                    </button>
                </div>
