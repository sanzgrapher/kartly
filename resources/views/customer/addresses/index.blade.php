        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-0 flex-1">
            ...
            <div class="md:col-span-3 bg-white dark:bg-gray-800 rounded-r-lg border border-l-0 border-gray-200 dark:border-gray-700 p-6">
                <div class="mb-8 flex justify-between items-center">
                    <div>
                        <h1 class="text-3xl font-semibold mb-2 dark:text-white">My Addresses</h1>
                ...
                @if ($addresses->count() > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @foreach ($addresses as $address)
                            <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-6 hover:shadow-lg transition dark:bg-gray-700/50">
                                <div class="flex justify-between items-start">
                                    <div>
                                        <p class="text-gray-800 font-medium dark:text-white">{{ $address->street_address_1 }}</p>
                                        @if ($address->street_address_2)
                                            <p class="text-sm text-gray-600 dark:text-gray-300">{{ $address->street_address_2 }}</p>
                                        @endif
                                        <p class="text-sm text-gray-600 dark:text-gray-300">{{ $address->city }}, {{ $address->state }} -
                                            {{ $address->country }}</p>
                                    </div>

                                    <div class="flex gap-2 items-center self-center">
                                        <a href="{{ route('addresses.edit', $address) }}"
                                            class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 font-medium text-sm">Edit</a>
                                        <form action="{{ route('addresses.destroy', $address) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 font-medium text-sm">Delete</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
